...
with open(file=coords, mode='r', encoding="utf-8") as chrom:
    ...
    for l in chrom:
        ...
        line = l.rstrip().split("\t")
        boundaries[line[0]]['end'] = int(line[1])  # tl_st: end of the initial telomere
...
for chrm, chrm_value in boundaries.items():

    start_timer_chrm = time()

    estimated_n_bins = int(chrm_value["end"] / resolution ) + 1
    pbar = tqdm(total = estimated_n_bins)

    print(f"chromosome {chrm.rsplit('r', 1)[1]} in progress. estimated number of bins is {estimated_n_bins}")
    ...
    while bin_start <= chrm_value['end']:
        ...
        if bin_end == boundaries[f"{chrm}"]['end']:
