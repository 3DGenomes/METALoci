...
def load_region_signals(mlobject: mlo.MetalociObject, signal_data: dict, signal_file: Path):
    ...
    region_signal = signal_data[mlobject.chrom][
    #marcius Problems with not exactly taking region
    #marcius    (signal_data[mlobject.chrom]["end"] >= int(mlobject.start))
    #marcius    & (signal_data[mlobject.chrom]["start"] <= int(mlobject.end))
    ...
    if len(region_signal) != len(mlobject.kk_coords):
        ...
        #marcius we cannot use nan otherwise all fails as LMI results in NaN
        #marcius tmp[:] = np.nan 
        tmp[:] = 0

        #marcius Maybe we need to use the following to avoid warning
        #marcius region_signal = region_signal.append(pd.DataFrame(tmp, columns=list(region_signal)), ignore_index=True)
